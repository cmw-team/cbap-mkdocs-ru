...
## Введение {: #http_receive_example_intro }
...
## Прикладная задача {: #http_receive_example_use_case }
...
## Порядок настройки получения HTTP-запросов {: #http_receive_example_sequence .pageBreakBefore }

--8<-- "http_receive_configure_sequence.md"

[](){: #connection_configure }
## Настройка подключения {: #http_receive_example_connection }
...
## Настройка пути передачи данных {: #http_receive_example_route .pageBreakBefore }
...
4. Настройте свойства пути передачи данных на следующих вкладках:
    ...
    - [**Основные свойства**](#http_receive_example_route_general)
    - [**Атрибуты сообщений**](#http_receive_example_route_attributes)
    - [**Интеграция**](#http_receive_example_route_integration)
...
- **Подключение** — выберите [подключение для получения HTTP-запросов](#http_receive_example_connection).
...
Добавить в файл /etc/nginx/sites-available/comindware\<instanceName> новый location для нового пути передачи данных
...
// Сноска, для каждого пути передачи данных на получение запросов надо добавить свой отдельный location

[](){: #scenario_configure }
## Настройка сценария {: #http_receive_example_scenario .pageBreakBefore }
...
2. У начального события измените тип на «**Получение сообщения**» и настройте его:
    ...
    - **Подключение:** [подключение для получения HTTP-запросов](#http_receive_example_connection)
    - **Путь передачи данных:** [путь передачи данных для получения HTTP-запросов](#http_receive_example_route)
...
[](){: #testing }
## Тестирование {: #http_receive_example_testing .pageBreakBefore }
...
1. С внешнего сервера отправьте в **{{ productName }}** запрос с данными заказов, например:
    ...
    - `https://<hostname>/api/public/adapter/uploadData` — **базовый путь получения HTTP-запросов** и **путь URI**, настроенные в [пути передачи данных](#http_receive_example_route);
